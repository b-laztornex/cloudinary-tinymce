...
module.exports = (grunt) => {
  ...
  grunt.initConfig({
    ...
    concat: {
      license: {
        ...
        files: {
          'dist/tiny-cloudinary-mce/plugin.js': [
            'src/text/license-header.js',
            scratchPluginMinPath
          ],
          'dist/tiny-cloudinary-mce/plugin.min.js': [
            'src/text/license-header.js',
            scratchPluginMinPath
          ]
        }
    ...
    copy: {
      css: {
        files: [
          { src: [ 'CHANGELOG.txt', 'LICENSE.txt' ], dest: 'dist/tiny-cloudinary-mce', expand: true }
        ]
      }
    },
  ...
  grunt.registerTask('version', 'Creates a version file', () => {
    grunt.file.write('dist/tiny-cloudinary-mce/version.txt', BUILD_VERSION);
  });
